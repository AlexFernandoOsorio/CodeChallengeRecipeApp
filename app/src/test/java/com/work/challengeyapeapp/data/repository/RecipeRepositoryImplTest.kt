...
class RecipeRepositoryImplTest {
    ...
    fun `when getRecipeListFromApi is called then verify it was called on RecipesApi`() =
        coroutineTestRule.runBlockingTest {
            // Given
            val expected = RecipeListResponse(
                listOf(
                    RecipeDto(
                        "1",
                        "Pork",
            ...
            coEvery { recipesApi.getRecipesListByLetter("pork") } returns expected

            // When
            val result = recipeRepositoryImpl.getRecipeListFromApi("pork")

            // Then
            coVerify { recipesApi.getRecipesListByLetter("pork") }
            Assert.assertEquals(expected.toModel(), result)
        }
